...
const mainJsPath = 'c:\\xampp\\htdocs\\Takahashi-s-Biblioteca-main\\Takahashi-s-Biblioteca\\main.js';
...
axios.defaults.baseURL = '/Takahashi-s-Biblioteca-main/Takahashi-s-Biblioteca/api';
        ...
        const login = async () => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // Disparar evento custom para el root
                    window.dispatchEvent(new Event('auth-changed'));
                    
                    alertify.success('Bienvenido ' + data.usuario.nombre);
...
const MainView = {
    ...
    setup() {
        ...
        const cargarCategorias = async () => {
            try { const { data } = await axios.get('/categorias.php'); categorias.value = data; } catch(e) {}
        };

        const usuarios = ref([]);
        const cargarUsuarios = async () => {
            if (rolActual.value !== 'admin') return;
            try { const { data } = await axios.get('/usuarios.php'); usuarios.value = data; } catch(e) {}
        };

        const libros = ref([]);
        const cargarLibros = async () => {
            try { const { data } = await axios.get('/libros.php'); libros.value = data; } catch(e) {}
        };

        const prestamos = ref([]);
        const cargarPrestamos = async () => {
             // Admin y Lector usan el mismo endpoint, el PHP filtra si es lector
            try { const { data } = await axios.get(\`/prestamos.php?usuario_id=\${simuladorLoginId.value}&rol=\${rolActual.value}\`); prestamos.value = data; } catch(e) {}
        };
        ...
        const formUsuario = ref({ id: null, nombre: '', email: '', password: '', rol: 'lector' });
        ...
        const cancelarEdicionUsuario = () => { formUsuario.value = { id: null, nombre: '', email: '', password: '', rol: 'lector' }; editModeUsuario.value = false; };
        const usuariosFiltrados = computed(() => {
            const qr = filtroUsuario.value.toLowerCase().trim();
            if (!qr) return usuarios.value;
            return usuarios.value.filter(u => u.nombre.toLowerCase().includes(qr) || u.email.toLowerCase().includes(qr));
        });
        ...
        const librosFiltrados = computed(() => {
            ...
            return libros.value.filter(l => l.titulo.toLowerCase().includes(qr) || l.isbn?.includes(qr) || l.autor.toLowerCase().includes(qr));
        });
        ...
        const renderChart = () => {
            ...
            setTimeout(() => {
                ...
                const counts = {};
                prestamos.value.forEach(p => { counts[p.libro_id] = (counts[p.libro_id] || 0) + 1; });
        ...
        onMounted(async () => {
            ...
            await cargarLibros();
            await cargarPrestamos();
            if (rolActual.value === 'admin') await cargarUsuarios();
            renderChart();
        });
        ...
        return {
            ...
            usuarios, filtroUsuario, editModeUsuario, formUsuario, guardarUsuario, editarUsuario, eliminarUsuario, cancelarEdicionUsuario, usuariosFiltrados, obtenerNombreUsuario,
            libros, filtroLibro, editModeLibro, formLibro, guardarLibro, editarLibro, eliminarLibro, cancelarEdicionLibro, librosFiltrados, obtenerTituloLibro, procesarImagenArchivo, buscarLibroISBN, obtenerNombreAutor,
            prestamos, editModePrestamo, formPrestamo, guardarPrestamo, editarPrestamo, eliminarPrestamo, cancelarEdicionPrestamo, devolverLibro, misPrestamos, autoPrestamo, guardarAnotacion, esAtrasado,
            // dummy variables to prevent errors in existing HTML since we removed autores
            autores: [], formAutor: {}, filtroAutor: '', editModeAutor: false, guardarAutor:()=>{}, editarAutor:()=>{}, cancelarEdicionAutor:()=>{}, eliminarAutor:()=>{}, autoresFiltrados: [] 
        };
    }
};
...
console.log("main.js refactored successfully.");
